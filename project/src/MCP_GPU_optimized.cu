...
__global__ void oneThreadPerEntry(i64 *dp_device, i64 *cut_device, int *p_device, int len, int N) {
    ...
    int i_row_dim = p_device[i];
    int j_col_dim = p_device[j + 1];

    if (i < N && j < N) {
        for (int k = i; k < j; ++k) {
            i64 cost = dp_device[convertIdx(k - i, i, N)] + dp_device[convertIdx(j - k - 1, k + 1, N)] +
                            1LL * i_row_dim * p_device[k + 1] * j_col_dim;
            if (cost < min_cost) {
                min_cost = cost;
                cut = k;
            }
    ...
    dp_device[convertIdx(len, i, N)] = min_cost;
    cut_device[convertIdx(len, i, N)] = cut;
}
